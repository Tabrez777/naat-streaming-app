...
const OtherSongsSection = ({ onPlay, songs = [] }) => {
  ...
  // ✨ ADVANCED UI: Mystic Violet Hover Aura + Levitating Card + Frosted Badge!
  const renderCard = (song, isGrid = false) => (
    <div 
      key={song.id} 
      onClick={() => onPlay(song)} 
      className={`group/card p-4 rounded-2xl bg-neutral-900/50 backdrop-blur-md hover:bg-neutral-800/90 transition-all duration-500 cursor-pointer border border-neutral-800/60 hover:border-purple-500/50 hover:-translate-y-1.5 hover:shadow-[0_12px_35px_rgba(168,85,247,0.22)] ${
    ...
    >
      <div className="relative aspect-square mb-4 rounded-xl overflow-hidden shadow-lg bg-neutral-800">
        ...
        {/* ✨ PRO UI: FLOATING MYSTIC VIOLET CATEGORY PILL */}
        <div className="absolute top-2.5 left-2.5 z-20 px-2.5 py-0.5 rounded-full text-[10px] font-extrabold tracking-wider uppercase bg-black/80 backdrop-blur-md text-purple-400 border border-purple-500/40 shadow-md">
          QAWWALI
        </div>
        ...
        <div className="absolute bottom-2 right-2 translate-y-6 opacity-0 group-hover/card:translate-y-0 group-hover/card:opacity-100 transition-all duration-300 ease-out z-10">
          <div className="w-12 h-12 bg-purple-500 rounded-full flex items-center justify-center hover:scale-110 active:scale-95 hover:bg-purple-600 shadow-xl shadow-purple-500/50 transition-transform">
            <svg className="w-6 h-6 text-white ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
          </div>
        </div>
      </div>
      <h3 className="text-white font-bold truncate text-base mb-1 group-hover/card:text-purple-400 transition-colors duration-300">{song.title}</h3>
      ...
      <div className="absolute top-0 left-0 w-full min-h-full z-[100] bg-[#121212] pb-32 animate-fade-in">
        <div className="sticky top-0 z-[110] bg-[#121212]/95 backdrop-blur-xl px-8 py-6 flex items-center gap-6 border-b border-white/10 shadow-2xl">
          ...
          <h1 className="text-white text-4xl font-extrabold tracking-tight flex items-center gap-3">
            Qawwalis
            <span className="text-xs px-3 py-1 bg-purple-500/20 text-purple-400 rounded-full border border-purple-500/30 font-bold uppercase tracking-widest">Collection</span>
          </h1>
        </div>
        
        <div className="p-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 relative z-[105]">
          {qawwalis.map(song => renderCard(song, true))}
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 relative group font-sans">
      <div className="flex items-center justify-between mb-6 pr-2">
        <h2 className="text-white text-2xl font-bold tracking-tight flex items-center gap-2.5">
          <span className="w-2 h-6 bg-purple-500 rounded-full shadow-[0_0_12px_rgba(168,85,247,0.8)]"></span>
          Qawwalis
        </h2>
        
        {qawwalis.length > 7 && (
          <button onClick={() => setShowAll(true)} className="group/btn flex items-center gap-2 px-4 py-2 rounded-full bg-neutral-900/80 hover:bg-neutral-800 transition-all duration-300 backdrop-blur-md border border-neutral-800 hover:border-purple-500/40 cursor-pointer shadow-lg hover:shadow-[0_0_15px_rgba(168,85,247,0.2)]">
            ...
            <svg className="w-4 h-4 text-neutral-400 group-hover/btn:text-purple-400 transition-all duration-300 group-hover/btn:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
          </button>
        )}
      </div>
