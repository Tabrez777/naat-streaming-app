...
const OtherSongsSection = ({ onPlay, songs = [] }) => {
  ...
  // ✨ FIX: Automatically scroll to top when opening the "Separate Page" view
  useEffect(() => {
    if (showAll) {
  ...
  // 🚀 REUSABLE ADVANCED SONG CARD (This was missing!)
    ...
    return (
      <div className="absolute top-0 left-0 w-full min-h-full z-[100] bg-[#121212] pb-32">
        <div className="sticky top-0 z-[110] bg-[#121212]/95 backdrop-blur-md px-8 py-6 flex items-center gap-6 border-b border-white/5 shadow-2xl">
          ...
          <h1 className="text-white text-4xl font-extrabold tracking-tight">Qawwalis</h1>
        </div>
        
        <div className="p-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 relative z-[105]">
          {/* ✨ FIX: Used qawwalis instead of sortedTilawats */}
  ...
  return (
    <div className="p-6 relative group font-sans">
      <div className="flex items-center justify-between mb-6 pr-2">
        <h2 className="text-white text-2xl font-bold tracking-tight">Qawwalis</h2>
        
        {/* ✨ FIX: Checked qawwalis.length instead of songs.length */}
        {qawwalis.length > 7 && (
