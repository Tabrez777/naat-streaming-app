...
const Sidebar = ({ playlists = [], setPlaylists, onPlaylistSelect, activeTab, setActiveTab, isOpen, onClose, user }) => {
  ...
  const [contextMenu, setContextMenu] = useState({ visible : false, x : 0, y : 0, playlist : null });
  const [renameModal, setRenameModal] = useState({ isOpen: false, playlistId: null, newName: "" });
  ...
  const handleRightClick = (e, playlist) => {
    ...
    let safeX = e.clientX;
    let safeY = e.clientY;
    if (e.clientX > window.innerWidth - 200) safeX = e.clientX - 200;
    if (e.clientY > window.innerHeight - 250) safeY = e.clientY - 180; 
    setContextMenu({ visible: true, x: safeX, y: safeY, playlist: playlist });
  };
  ...
  const handlePin = () => {
    const updated = playlists.map(p => p.id === contextMenu.playlist.id ? { ...p, isPinned: !p.isPinned } : p);
    syncToFirebase(updated);
  };

  const handleRename = () => {
    setRenameModal({ isOpen: true, playlistId: contextMenu.playlist.id, newName: contextMenu.playlist.name });
  ...
  const submitRename = () => {
    ...
    const updated = playlists.map(p => p.id === renameModal.playlistId ? { ...p, name: renameModal.newName } : p);
    syncToFirebase(updated);
    setRenameModal({ isOpen: false, playlistId: null, newName: "" });
  };
  ...
  return (
    <>
      {/* ✨ FIX 1: Mobile Backdrop - Tapping anywhere outside the sidebar on mobile closes it */}
      {isOpen && (
        <div 
      ...
      <aside className={`w-64 h-[100dvh] text-white flex flex-col pt-3 pb-28 md:pb-6 px-4 select-none border-r border-neutral-900 fixed md:relative top-0 left-0 z-50 transition-transform duration-300 ease-in-out bg-black md:bg-transparent ${
        ...
        <div className="flex flex-col gap-1 mb-2">
         <div className="flex items-center gap-4 px-2 mb-4 h-12 flex-shrink-0 md:hidden">
          ...
          <button 
            onClick={() => { navigate('/'); setActiveTab("Home"); onPlaylistSelect(null); onClose(); }}
          ...
          >
            <svg className="w-6 h-6 fill-current" viewBox="0 0 24 24"><path d="M4 10V21h6v-6h4v6h6V10l-8-7z" /></svg>
            Home
          </button>

          <button 
            onClick={() => { navigate('/settings'); setActiveTab('Settings'); onClose(); }}
            className={`flex cursor-pointer items-center gap-6 px-4 py-3 rounded-xl text-sm font-medium transition-colors duration-150 ${
        ...
        <div className="flex-1 overflow-y-auto px-2 flex flex-col gap-4 mt-3 scrollbar-hide">
          {sortedPlaylists.map((playlist) => (
            <div 
              key={playlist.id}
              onClick={() => { navigate(`/playlist/${playlist.id}`); onPlaylistSelect(playlist); onClose(); }}
            ...
            >
              <div className="flex flex-col">
                <span className="text-sm font-medium text-neutral-200 group-hover:text-white transition-colors truncate w-48">
        ...
        {contextMenu.visible && (
          ...
          >
            ...
            <button onClick={handleRename} className="w-full text-left px-4 py-2 hover:bg-neutral-700 hover:text-white transition">Rename</button>
            <button onClick={handleShare} className="w-full text-left px-4 py-2 hover:bg-neutral-700 hover:text-white transition border-b border-neutral-700">Share Link</button>
            <button onClick={handleDelete} className="w-full text-left px-4 py-2 hover:bg-neutral-700 text-red-400 hover:text-red-300 transition">Delete</button>
          </div>
        )}
      </aside>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
          <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl shadow-2xl w-full max-w-xs sm:max-w-sm transform transition-all">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setRenameModal({ isOpen: false, playlistId: null, newName: "" })} className="flex-1 py-2.5 rounded-full font-medium text-neutral-400 hover:text-white hover:bg-neutral-800 transition-colors">Cancel</button>
              <button onClick={submitRename} className="flex-1 py-2.5 rounded-full font-bold bg-[#1ed760] text-black hover:bg-[#1fdf64] hover:scale-105 transition-all">Save</button>
            </div>
          </div>
        </div>
      )}
