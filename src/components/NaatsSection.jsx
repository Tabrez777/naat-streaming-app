...
const NaatsSection = ({ songs = [], onPlay }) => {
  ...
  // ✨ ADVANCED UI: Levitating cards + Emerald Neon Hover Aura + Frosted Category Badge!
  const renderCard = (song, isGrid = false) => (
    <div 
      key={song.id} 
      onClick={() => onPlay(song)} 
      className={`group/card p-4 rounded-2xl bg-neutral-900/50 backdrop-blur-md hover:bg-neutral-800/90 transition-all duration-500 cursor-pointer border border-neutral-800/60 hover:border-[#1ed760]/50 hover:-translate-y-1.5 hover:shadow-[0_12px_35px_rgba(30,215,96,0.18)] ${
    ...
    >
      <div className="relative aspect-square mb-4 rounded-xl overflow-hidden shadow-lg bg-neutral-800">
        ...
        {/* ✨ PRO UI: FLOATING FROSTED GLASS CATEGORY PILL */}
        <div className="absolute top-2.5 left-2.5 z-20 px-2.5 py-0.5 rounded-full text-[10px] font-extrabold tracking-wider uppercase bg-black/80 backdrop-blur-md text-[#1ed760] border border-[#1ed760]/40 shadow-md">
          NAAT
        </div>
        ...
        <div className="absolute bottom-2 right-2 translate-y-6 opacity-0 group-hover/card:translate-y-0 group-hover/card:opacity-100 transition-all duration-300 ease-out z-10">
          <div className="w-12 h-12 bg-[#1ed760] rounded-full flex items-center justify-center hover:scale-110 active:scale-95 hover:bg-[#1fdf64] shadow-xl shadow-[#1ed760]/50 transition-transform">
      ...
      <div className="absolute top-0 left-0 w-full min-h-full z-[100] bg-[#121212] pb-32 animate-fade-in">
        <div className="sticky top-0 z-[110] bg-[#121212]/95 backdrop-blur-xl px-8 py-6 flex items-center gap-6 border-b border-white/10 shadow-2xl">
          ...
          <h1 className="text-white text-4xl font-extrabold tracking-tight flex items-center gap-3">
            Naat-e-Paak
            <span className="text-xs px-3 py-1 bg-[#1ed760]/20 text-[#1ed760] rounded-full border border-[#1ed760]/30 font-bold uppercase tracking-widest">Collection</span>
          </h1>
        </div>
    ...
    <div className="p-6 relative group font-sans">
      <div className="flex items-center justify-between mb-6 pr-2">
        <h2 className="text-white text-2xl font-bold tracking-tight flex items-center gap-2.5">
          <span className="w-2 h-6 bg-[#1ed760] rounded-full shadow-[0_0_12px_#1ed760]"></span>
          Naat-e-Paak
        </h2>
        
        {songs.length > 7 && (
          <button onClick={() => setShowAll(true)} className="group/btn flex items-center gap-2 px-4 py-2 rounded-full bg-neutral-900/80 hover:bg-neutral-800 transition-all duration-300 backdrop-blur-md border border-neutral-800 hover:border-[#1ed760]/40 cursor-pointer shadow-lg hover:shadow-[0_0_15px_rgba(30,215,96,0.15)]">
            ...
            <svg className="w-4 h-4 text-neutral-400 group-hover/btn:text-[#1ed760] transition-all duration-300 group-hover/btn:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
          </button>
        )}
      </div>
